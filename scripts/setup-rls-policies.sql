...
-- AI PROCESSING JOB POLICIES
CREATE POLICY "Users can access AI jobs in their tenant" ON ai_processing_jobs
  FOR SELECT USING (tenant_id = auth.tenant_id());

CREATE POLICY "Users can create AI jobs in their tenant" ON ai_processing_jobs
  FOR INSERT WITH CHECK (tenant_id = auth.tenant_id());

CREATE POLICY "System can update AI jobs" ON ai_processing_jobs
  FOR UPDATE USING (tenant_id = auth.tenant_id());
...
CREATE POLICY "Users can access shares in their tenant" ON shares
  FOR SELECT USING (
    ...
    shared_with_user_id = auth.uid()::uuid
  );
...
CREATE POLICY "Users can update their own shares" ON shares
  FOR UPDATE USING (
    tenant_id = auth.tenant_id() AND 
    shared_by_user_id = auth.uid()::uuid
  );
