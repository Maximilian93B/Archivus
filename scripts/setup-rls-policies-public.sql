-- Archivus RLS (Row Level Security) Setup - Public Schema Version
...
-- Helper function to extract tenant_id from JWT (in public schema)
...
CREATE POLICY "Users can update their own profile" ON users
  FOR UPDATE USING (tenant_id = public.get_tenant_id() AND id = auth.uid()::uuid);

CREATE POLICY "Admins can manage users in their tenant" ON users
  FOR ALL USING (tenant_id = public.get_tenant_id() AND public.is_user_admin());
...
CREATE POLICY "Users can update their own documents or admins can update any" ON documents
  FOR UPDATE USING (
    ...
    (created_by = auth.uid()::uuid OR public.is_user_admin())
  );
...
CREATE POLICY "Users can access document versions in their tenant" ON document_versions
  FOR SELECT USING (
    EXISTS (
      ...
      AND d.tenant_id = public.get_tenant_id()
    )
  );
...
CREATE POLICY "Users can access workflow tasks in their tenant" ON workflow_tasks
  FOR SELECT USING (
    EXISTS (
      ...
      AND w.tenant_id = public.get_tenant_id()
    )
  );
