Write-Host "ARCHIVUS PHASE 2 TEST - FIXED VERSION" -ForegroundColor Cyan
...
try {
    ...
    if ($docs.documents) {
        ...
        $docCount = $docs.data.Count
    } elseif ($docs -is [array]) {
        $docCount = $docs.Count
    }
    Write-Host "   Found $docCount documents" -ForegroundColor White
} catch {
...
"@
    ...
    # Handle different response structures for document ID
    $documentId = $null
...
# Test 5: Document Retrieval (only if we got a document ID)
if ($documentId) {
    ...
    Write-Host "`n5-6. Skipping document retrieval and download tests (no document ID)" -ForegroundColor Yellow
}
...
Write-Host "Authentication and basic functionality working!" -ForegroundColor Green 